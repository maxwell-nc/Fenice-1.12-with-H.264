...
int get_SDP_descr(media_entry * media, char *descr, int extended, char *url)
{
	...
	sprintf(descr + strlen(descr), "i=%s %s Streaming Server" SDP_EL,
		PACKAGE, VERSION);
	...
	if (p->flags & ME_AGGREGATE) {
		sprintf(descr + strlen(descr), "a=control:%s!%s" SDP_EL, url,
			p->aggregate);
	}
	...
	do {
		...
		if (p->description.payload_type >= 96) {
			...
			if (strcmp(p->description.encoding_name, "MP4V-ES") ==
			    0) {
